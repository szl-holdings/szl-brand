...
def _text(
    ...
    *,
    ...
    return (
        ...
        f'font-size="{size}"{weight_attr}>{html.escape(value)}</text>'
    )
...
def _rect(
    ...
    *,
    ...
    return (
        ...
        f'fill="{fill}" stroke="{stroke}"{opacity_attr}/>'
    )
...
def _path(
    ...
    *,
    ...
    return (
        ...
        f'stroke-width="{width}"{opacity_attr}/>'
    )
    ...
    return "".join(
        (
            ...
            _text(
                64,
                178,
                f"{profile.interaction} · {profile.evidence_placement}",
                size=13,
            ),
        )
    )
...
def _product_body(profile: MotifProfile, seed: bytes) -> str:
    pieces = [
        ...
        _path(
            "M92 430 C180 340 255 446 342 318 S490 366 536 286",
...
def _runtime_body(profile: MotifProfile, seed: bytes) -> str:
    pieces = [
        _rect(960, 226, 250, 300, rx=16, fill="#050C14"),
        _text(986, 254, "TOPOLOGY"),
    ]
    for index in range(6):
        y = 250 + index * 46
        ...
        pieces.extend(
            (
                ...
                _text(
                    226,
                    y + 5,
                    f"RECEIPT-{index + 1} · sha256:{digest}",
                    fill="#D7E0EA",
                ),
            )
        )
...
def _archive_body(seed: bytes) -> str:
    ...
    for index in range(6):
        ...
        pieces.extend(
            (
                _circle(x, y, 7, fill="#9AA7BD"),
                _path(f"M{x + 7} {y} H{x + 175}", stroke="#738097"),
            )
        )
    return "".join(pieces)
...
def render_svg(profile: MotifProfile) -> str:
    ...
    description = html.escape(
        "SZL Holdings "
        f"{profile.theme_family} motif. Interaction family: {profile.interaction}. "
        f"Evidence placement: {profile.evidence_placement}. Variant {profile.variant}."
    )
    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="{WIDTH}" height="{HEIGHT}" viewBox="0 0 {WIDTH} {HEIGHT}" role="img" aria-labelledby="szl-title szl-desc">
<title id="szl-title">{title} — {profile.theme_family}</title>
<desc id="szl-desc">{description}</desc>
...
def build_bundle(
    ...
    """Build the complete deterministic in-memory artifact bundle."""
...
def write_bundle(request: MotifRequest, output: str | Path) -> dict[str, Path]:
    """Write exactly the four declared artifacts into one output directory."""
...
def generate_motifs(requests: list[MotifRequest], output: Path) -> list[Path]:
    ...
    for request in requests:
        ...
        paths = write_bundle(request, directory)
        created.extend(paths.values())
    return created
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="szl-motif",
        description=(
            "Generate class-native SZL motif assets from reviewed estate classes."
        ),
    )
